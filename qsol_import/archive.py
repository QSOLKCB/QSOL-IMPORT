...
def validate_member(info: zipfile.ZipInfo) -> None:
    ...
    if info.filename.startswith(("/", "\\")) or path.is_absolute() or ".." in path.parts:
...
def validate_archive(zf: zipfile.ZipFile, policy: dict) -> None:
    ...
    for info in members:
        ...
        if info.compress_size > 0 and info.file_size / info.compress_size > float(limits["max_compression_ratio"]):
