...
class RoadmapCompletionTests(unittest.TestCase):
    ...
    def test_completion_contract_schemas_are_strict_json_with_unique_ids(self):
        names = [
            ...
            "external-execution-receipt.schema.json",
    ...
    def test_clean_room_nested_result_contracts_are_closed(self):
        schema = loads_strict(
            (ROOT / "schemas/ark-cleanroom-receipt.schema.json").read_bytes()
        )
        ...
        transport_objects = properties["transport_equivalence"]["properties"][
            "objects"
        ]["items"]
        self.assertFalse(transport_objects["additionalProperties"])
        self.assertEqual(
            properties["clean_room"]["properties"]["portable_inputs_only"],
            {"const": True},
        )

    def test_retention_obligations_require_authority_and_self_receipt(self):
        schema = loads_strict(
            (ROOT / "schemas/retention-obligations.schema.json").read_bytes()
        )
        self.assertIn("authority", schema["required"])
        self.assertIn("obligations_sha256", schema["required"])
        self.assertEqual(
            schema["properties"]["authority"]["const"],
            "measurement-obligations-only",
        )
